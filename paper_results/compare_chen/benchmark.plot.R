...
accuracy <- function(){
    ...
    p <- ggplot(dt, aes(x=momi,y=Chen,color=n)) + geom_abline(color='red', linetype='dashed') + geom_point() + scale_y_continuous(trans = sgn_log_trans()) + scale_x_continuous(trans = sgn_log_trans(), labels=function(x) as.character(round(x,2)))
    plot(p)
