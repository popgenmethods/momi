...
dt$n <- dt$D * dt$n_per_pop
dt$D <- as.factor(dt$D)

dt$method[dt$method == 'chen'] <- 'Chen'
dt$method[dt$method == 'moran'] <- 'momi'
...
dt2 <- dcast(dt, n + D + site + run_id + state + demography + initial ~ method, value.var = "result")
