...
timing <- function(){
    ...
    levels(table$N)[1] <- 'momi'
    p <- ggplot(table, aes(x=n,y=seconds,color=N,linetype=N)) + scale_y_log10() + geom_point() + geom_line()
    ggsave('timing.pdf',p)
}
...
accuracy <- function(){
    ...
    p <- ggplot(table, aes(x=momi,y=abs.ratio, color=sign)) + geom_point() + geom_abline(intercept=0,slope=0) + scale_x_log10() + scale_y_log10() + ylab('|dadi/momi|') + facet_grid(N~n, labeller=label_both) + guides(color=guide_legend(title="sign (dadi)"))
